...
Use estas regras ao trabalhar com componentes da suíte **Projeto ACBr** (Automação Comercial Brasil) em aplicações FreePascal/Lazarus.

## Isolamento e Padrões (SOLID)

- ❌ **Não instancie ou acople os componentes ACBr diretamente nos formulários de UI** (ex: colocar um `TACBrNFe` dentro do `TfrmEmissao.lfm`).
