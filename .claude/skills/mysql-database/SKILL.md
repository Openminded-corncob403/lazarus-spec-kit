---
name: "MySQL Database"
description: "Padrões de desenvolvimento com MySQL/MariaDB via SQLdb/Zeos — conexão, stored procedures, AUTO_INCREMENT, JSON, triggers, replication, migrations"
---

# MySQL Database — Skill

Use esta skill ao trabalhar com banco de dados MySQL ou MariaDB em projetos FreePascal/Lazarus via SQLdb/Zeos.

## Quando Usar

- Ao configurar conexão SQLdb/Zeos com MySQL ou MariaDB
- Ao criar tabelas, stored procedures, functions, triggers e views
- Ao implementar Repositories com SQLdb/Zeos + MySQL
...
uses
  SQLdb/Zeos.Comp.Client,
  SQLdb/Zeos.Phys.MySQL,       // Driver MySQL
  SQLdb/Zeos.Phys.MySQLDef,    // Defaults do MySQL
  SQLdb/Zeos.Stan.Def,
  SQLdb/Zeos.DApt;

type
  /// <summary>
  ///   Factory de conexão MySQL via SQLdb/Zeos.
  /// </summary>
...
```pascal
uses
  SQLdb/Zeos.Phys.MySQLWrapper,
  SQLdb/Zeos.Phys.MySQL;
...
## Tipos de Dados — Mapeamento MySQL ↔ FreePascal/Lazarus

| MySQL | FreePascal/Lazarus (SQLdb/Zeos) | Observação |
|-------|------------------|------------|
...
### Obter o ID Gerado no FreePascal/Lazarus
...
begin
  ...
  try
    ...
    { Método 2: Via propriedade SQLdb/Zeos (mais direto) }
...
**No FreePascal/Lazarus (mapear para enum Pascal):**
...
```pascal
procedure ExecuteInTransaction(AConnection: TSQLConnection; AProc: TProc);
begin
...
## Migração de Schema no FreePascal/Lazarus

```pascal
/// <summary>
///   Verifica se uma tabela existe no MySQL.
/// </summary>
function TableExists(AConnection: TSQLConnection;
...
/// </summary>
function ColumnExists(AConnection: TSQLConnection;
...
-- MySQL < 8.0: gerar no FreePascal/Lazarus e enviar como parâmetro
```
...
{ Gerar UUID no FreePascal/Lazarus para MySQL < 8.0 }
