...
description: "Padrões de desenvolvimento com PostgreSQL via SQLdb/Zeos — conexão, PL/pgSQL, sequences, JSONB, UPSERT, full-text search, migrations"
---

# PostgreSQL Database — Skill

Use esta skill ao trabalhar com banco de dados PostgreSQL em projetos FreePascal/Lazarus via SQLdb/Zeos.

## Quando Usar

- Ao configurar conexão SQLdb/Zeos com PostgreSQL
- Ao criar tabelas, sequences, functions, triggers e views
- Ao implementar Repositories com SQLdb/Zeos + PostgreSQL
...
## Conexão SQLdb/Zeos com PostgreSQL
...
uses
  ...
  SQLdb/Zeos.Phys.PG,         // Driver PostgreSQL
  SQLdb/Zeos.Phys.PGDef,      // Defaults do PostgreSQL
  SQLdb/Zeos.Stan.Def,
  SQLdb/Zeos.Stan.Pool,
  SQLdb/Zeos.DApt;

type
  /// <summary>
  ///   Factory de conexão PostgreSQL via SQLdb/Zeos.
  /// </summary>
  ...
  public
    ...
    /// <returns>Conexão SQLdb/Zeos configurada e aberta</returns>
...
class function TPostgreSQLConnectionFactory.CreateFromConnectionString(
  const AConnectionString: string): TSQLConnection;
begin
  Result := TSQLConnection.Create(nil);
  try
...
## Tipos de Dados — Mapeamento PostgreSQL ↔ FreePascal/Lazarus

| PostgreSQL | FreePascal/Lazarus (SQLdb/Zeos) | Observação |
|------------|------------------|------------|
...
### RETURNING no FreePascal/Lazarus
...
```sql
-- Function que retorna valor (equivale a function no FreePascal/Lazarus)
...
```pascal
{ No FreePascal/Lazarus: escutar eventos via SQLdb/Zeos Event Alerter }
uses
  SQLdb/Zeos.Phys.PG;
...
/// </summary>
function TableExists(AConnection: TSQLConnection; const ATableName: string;
