...
description: "Padrões de desenvolvimento com banco de dados Firebird via SQLdb/Zeos — conexão, PSQL, generators, transactions, migrations"
---

# Firebird Database — Skill

Use esta skill ao trabalhar com banco de dados Firebird em projetos FreePascal/Lazarus via SQLdb/Zeos.

## Quando Usar

- Ao configurar conexão SQLdb/Zeos com Firebird
- Ao criar tabelas, generators, stored procedures, triggers, domains e views
- Ao implementar Repositories com SQLdb/Zeos + Firebird
...
## Conexão SQLdb/Zeos com Firebird
...
uses
  ...
  SQLdb/Zeos.Phys.FB,        // Driver Firebird
  SQLdb/Zeos.Phys.FBDef,     // Defaults do Firebird
  SQLdb/Zeos.Stan.Def,
...
type
  /// <summary>
  ///   Factory de conexão Firebird via SQLdb/Zeos.
  /// </summary>
...
class function TFirebirdConnectionFactory.CreateConnection(
  ...
  const APassword: string): TSQLConnection;
begin
...
class function TFirebirdConnectionFactory.CreateEmbeddedConnection(
  const ADatabasePath: string): TSQLConnection;
begin
  Result := TSQLConnection.Create(nil);
  try
    Result.DriverName := 'FB';
...
## Tipos de Dados — Mapeamento Firebird ↔ FreePascal/Lazarus

| Firebird | FreePascal/Lazarus (SQLdb/Zeos) | Observação |
|----------|------------------|------------|
...
### Obter Próximo Valor no FreePascal/Lazarus
...
### RETURNING no FreePascal/Lazarus (pegar ID após Insert)
...
**Chamar no FreePascal/Lazarus (tratada como SELECT):**
...
```pascal
{ No FreePascal/Lazarus: escutar eventos do banco }
uses
  SQLdb/Zeos.Phys.FB; // TFDPhysFBEventAlerter
...
/// </summary>
procedure EnsureMigrationTable(AConnection: TSQLConnection);
begin
  AConnection.ExecSQL(
...
function TableExists(AConnection: TSQLConnection; const ATableName: string): Boolean;
var
  LQuery: TSQLQuery;
begin
  LQuery := TSQLQuery.Create(nil);
